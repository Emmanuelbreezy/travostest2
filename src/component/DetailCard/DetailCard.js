...
export const DetailCard = (props) => {
  return (
	  <div className="w-full p-2" style={{ backgroundColor: props.itemcolor }}>
       <div className="flex flex-col items-center justify-between">
		   <div className="p-3">
		        <div>
				  <img className="imag object-none mt-2" src={props.image} />
				</div>
		   ...
		   <div className="w-full px-3 flex items-center justify-between mb-7">
			  <div className="flex-1">
					  <h2 className="text-black font-bold text-lg">{props.toptitle}</h2>
			  	<ul className="flex items-center space-x-2">
						  <h4 className="font-semibold text-gray-800">N {props.amount}</h4>
						  <span className="text-xs font-bold text-gray-700">{props.subdesc}</span>
			  	</ul>
			  </div>
			  <div className="flex-shrink-0 pr-2">
				  <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-6 fill-current" viewBox="0 0 24 24">
